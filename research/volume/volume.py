    ...
    for i in range(bbox[0], bbox[2]+1):
        for j in range(bbox[1], bbox[3]+1):
            if (cv2.pointPolygonTest(points, (i, j), False) >= 0):
                volume += Max(0, (lowest - img[j][i]) * depth_per_pix -
                              plate_thickness) * len_per_pix * len_per_pix
    ...
    with open(json_path, 'r') as json_file:
        ...
        for shape in data['shapes']:
            if (shape['label'] == "plate"):
                len_per_pix, depth_per_pix = get_scale(
                    shape['points'], img, lowest)
        ...
        for shape in data['shapes']:
            ...
            if (label == "plate"):
                continue
            points = shape['points']
            volume = cal_volume(points, img, len_per_pix,
                                depth_per_pix, lowest)
